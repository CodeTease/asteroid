...
def verify_void_ui():
    with sync_playwright() as p:
        ...
        # 4. Kill Boss to unlock Void Mode
        page.evaluate("game.finalBoss.health = 0")
        time.sleep(2)

        # 5. Close Upgrade Modal if it appeared
        if page.locator("#upgrade-modal").is_visible():
            page.locator("#continue-button").click()
            time.sleep(1)

        # 6. Fast Forward Time to 100s IN VOID MODE
        # We need to advance gameTime so that (gameTime - voidStartTime) >= 100
        # voidStartTime was set when boss died (approx t=current).
        # So we need to add 100 to current gameTime.
